// Sherd White Digital Image Processing Assignment #3.6
...
int main(int argc, char * argv[])
{
	...
	int height = img.rows, width = img.cols, x = 0, y = 0;
...
void equalizeGrayscaleROI(Mat img, int x, int y, int height, int width)
{
	...
	for (int r = y; r < (y + height); r++)
	{
		for (int c = x; c < (x + width); c++)
		{
			grayimg.at<uchar>(r, c) -= img.at<uchar>(r, c);
		}
	}

	namedWindow("Grayscale Image Subtraction", WINDOW_AUTOSIZE);
	imshow("Grayscale Image Subtraction", grayimg);
}

void equalizeColorROI(Mat colorimg, int x, int y, int height, int width)
{
	std::cout << "This is a color image." << std::endl;
	Mat originalimg = colorimg.clone();
	...
	for (int r = y; r < (y + height); r++)
		for (int c = x; c < (x + width); c++)
		{
			colorimg.at<Vec3b>(r, c)[0] -= originalimg.at<Vec3b>(r, c)[0];
			colorimg.at<Vec3b>(r, c)[1] -= originalimg.at<Vec3b>(r, c)[1];
			colorimg.at<Vec3b>(r, c)[2] -= originalimg.at<Vec3b>(r, c)[2];
		}

	namedWindow("Color Image Subtraction", WINDOW_AUTOSIZE);
	imshow("Color Image Subtraction", colorimg);
}
